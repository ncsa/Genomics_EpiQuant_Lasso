...
object test {
  def main(args: Array[String]) {
    ...
    val n = snp_names.length
     ...
     val solver = new ADMM()
    val fit = solver.fit(dataset, m, n*(n+1)/2, num_part, 100, 1.0, 1e-4, 1e-2)
...
class ADMM extends Lasso {
  ...
  def main(args: Array[String]) {
    ...
    // val solver = new ADMM()
    fit(dataset, 0, n * (n + 1) / 2, MAX_ITER, LAMBDA, RHO, ABSTOL, RELTOL)
  ...
  */
  ...
  def fit(dataset: RDD[(BDM[Double], BDM[Double], Int)], Phe: Int, D: Int, maxIter: Int, lambda: Double, rho: Double, absTol: Double, relTol: Double) {
    ...
    var cholesky_A = dataset.mapPartitions {
      iterator => {
        val g: (BDM[Double], BDM[Double], Int) = iterator.next
        val snp_matrix = g._1
        val phe_matrix = g._2
